...
export class PostulerComponent implements OnInit {
  ...
  data: any ;
  idPost:any
  constructor(private postulS: PostulerService, private service: OffreService,private acttivatedRoot:ActivatedRoute) {
    ...
    this.addForm=new FormGroup({
      ...
      "datePostulation" : new   FormControl(''),
  ...
  postulerOffre(){
    ...
    if(this.addForm.valid){
      this.postulS.postulerOffre(formData).subscribe((res)=>{
  ...
  updatePostulation(id: number, data : any){
    this.postulS.updatePostulation(id, data).subscribe((res)=>{
      console.log(res);
